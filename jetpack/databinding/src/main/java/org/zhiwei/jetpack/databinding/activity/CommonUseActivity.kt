...
import org.zhiwei.jetpack.databinding.adapter.LAdapter
import org.zhiwei.jetpack.databinding.adapter.RAdapter
import org.zhiwei.jetpack.databinding.bean.CommonUser
import org.zhiwei.jetpack.databinding.bean.FieldUser
import org.zhiwei.jetpack.databinding.bean.ObUser
import org.zhiwei.jetpack.databinding.databinding.ActivityCommonUseBinding
...
class CommonUseActivity : AppCompatActivity(), View.OnClickListener {
	...
	private val tvField: AppCompatTextView by lazy { findViewById(R.id.tv_field_user_info) }
...
//        在xml中则直接用user.desc即可，不需要写.get()。写了也无妨
		binding.fuser = fuser
		obuser =
			ObUser("王二", 22, 0, "你会看到，这个王二的性别，你是改不了的，因为内部val声明了不可变量")
	...
	override fun onClick(view: View?) {
		...
		when (view?.id) {
			R.id.btn_change_common -> {
				...
				Toast.makeText(this, "注意上面的common user的info信息不会变化", Toast.LENGTH_SHORT)
					.show()
